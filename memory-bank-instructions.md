...
Because this is a large-scale project, the memory bank is loaded in stages. The `.kilocode/rules/memory-bank` folder, the top-level memory bank, is mandatory reading. Additionally, the `.kilocode/sub-memory-bank` folder contains memories for various business modules. I will load the subfiles based on the index defined in `architecture.md` and load them as needed.
...
### Optional Files
Create additional files/folders within sub-memory-bank/ when they help organize:
- `module-requirements.md` - Detailed requirements for specific business modules
- Module-specific data models
- Business logic implementations
- UI/UX guidelines per module
